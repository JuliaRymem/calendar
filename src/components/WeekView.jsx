import { addDays, startOfWeek, parseISO, isBefore, isAfter, startOfDay, endOfDay, isSameDay, format } from "date-fns";
...
function layoutLanes(items) {
  ...
  for (const ev of placed) {
    ...
    while (true) {
      ...
      if (!conflict) { (lanes[lane] ||= []).push(ev); ev.lane = lane; break; }
      lane++;
    }
  }
...
export default function WeekView() {
  ...
  function beginCreate(dayIndex, e) {
    ...
    draftRef.current = { dayIndex, start, end: new Date(start), color: "#6366f1", label: "Övrigt" };
  ...
  function onBlockDragStart(event, mdEvent, dayIndex) {
    const col = containerRefs.current[dayIndex]; const rect = col.getBoundingClientRect();
    dragState.current = { id: event.id, type: "move", rectTop: rect.top, startY: mdEvent.clientY, baseStart: new Date(event.start), baseEnd: new Date(event.end), dayIndex };
  ...
  function onBlockResizeStart(event, edge, mdEvent, dayIndex) {
    const col = containerRefs.current[dayIndex]; const rect = col.getBoundingClientRect();
    dragState.current = { id: event.id, type: edge, rectTop: rect.top, startY: mdEvent.clientY, baseStart: new Date(event.start), baseEnd: new Date(event.end), dayIndex };
    window.addEventListener("mousemove", onDragMove); window.addEventListener("mouseup", onDragEnd);
  }
  function onDragMove(e) {
    const s = dragState.current; if (!s) return;
    const dy = e.clientY - s.rectTop - 28; const base = days[s.dayIndex];
    ...
    if (s.type === "move") {
      ...
      updateEvent(s.id, { start: new Date(s.baseStart.getTime() + deltaMin * 60000).toISOString(), end: new Date(s.baseEnd.getTime() + deltaMin * 60000).toISOString() });
  ...
  function onDragEnd() { window.removeEventListener("mousemove", onDragMove); window.removeEventListener("mouseup", onDragEnd); dragState.current = null; }
    ...
    <div>
      ...
      <div className="grid grid-cols-[auto_repeat(7,minmax(0,1fr))]">
        ...
        {days.map((day, idx) => {
          ...
          return (
            <div key={day} ref={(el) => (containerRefs.current[idx] = el)}
                 className={`relative col-border ${isToday ? "bg-red-50 dark:bg-red-900/20" : ""}`}
                 onMouseDown={(e) => beginCreate(idx, e)}>
              <div className={`sticky-head px-2 py-1 text-center text-sm font-medium ${isToday ? "text-red-600 dark:text-red-400" : ""}`}>
              ...
              {Array.from({ length: 24 }, (_, h) => (<div key={h} className="h-16 grid-line" />))}
