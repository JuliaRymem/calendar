import {
  ...
  export default function DayView() {
    ...
    const timed = useMemo(
      () =>
        filtered
          .filter((e) => !e.allDay && overlapsDay(e, selectedDate))
    ...
    function onResizeStart(ev, edge) {
      ...
      const move = (mm) => {
        const yy = mm.clientY - rect.top;
        const current = yToDate(selectedDate, Math.max(0, Math.min(yy, 24 * HOUR_PX)));
        if (edge === "start" && current < parseISO(ev.end))
          updateEvent(ev.id, { start: current.toISOString() });
        if (edge === "end" && current > parseISO(ev.start))
          updateEvent(ev.id, { end: current.toISOString() });
      };
      const up = () => {
        window.removeEventListener("mousemove", move);
        window.removeEventListener("mouseup", up);
      ...
      <div className="relative overflow-hidden rounded-xl border">
        ...
        <div className="flex items-center justify-between border-b bg-white px-4 py-2 dark:bg-zinc-900">
          ...
          <div
            className={`text-lg font-medium capitalize ${
              isSameDay(selectedDate, new Date()) ? "text-red-600" : ""
            }`}
          >
        ...
        <div className="relative" ref={containerRef} onMouseDown={onBackgroundMouseDown}>
          {hours.map((h) => (
            <div
              key={h}
              className="relative h-16 border-b border-gray-200 dark:border-zinc-800"
            >
